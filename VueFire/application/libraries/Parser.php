...
class parser
{
  ...
  function _include_content( $matches )
  {
    ...
    if ( $this->include->run_parser )
    {
      ...
      return $parser->parse( $file, $this->input->data, array( 'show' => FALSE, 'append' => $this->data->append, 'is_string' => FALSE, 'clean' => FALSE ));
    }
    ...
    foreach ( $matches[0] as $id => $original )
    {
      ...
      if ( ! empty( $value->arguments ) && $this->config->allow_modificators )
      {
        ...
        foreach( $value->arguments as $argument )
        {
          ...
          {
            $parameters[2] = str_replace( '&', $value->result , $parameters[2] );

            preg_match_all("#[\'\"]*([^\'\" ]+)[\'\"]*,?#", $parameters[2], $parameter);
          ...
          if (is_callable( $fn ))
          {
            if ( ($value->result = call_user_func_array( $fn, $parameter )) === FALSE)
            {
